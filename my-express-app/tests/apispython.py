...
def search_and_match(song_name):
    try:
        ...
        spotify_tracks_results = sp.search(q=song_name, type='track', limit=1)
...
def create_song():
    ...
    if not data or 'track_spotify_id' not in data:
        return jsonify({'message': 'Missing song parameter'}), 400
