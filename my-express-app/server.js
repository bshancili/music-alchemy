...
app.post("/temporal_recommendation", async (req, res) => {
  ...
  try {
    const userData = { uid: userUid };
    const response = await axios.post("http://localhost:3000/recent_liked_songs", userData);
    ...
    let songDetailsList = await Promise.all(
      songIds.map(async (songId) => {
        const songData = { docId: songId };
        const songNameResponse = await axios.post("http://localhost:3000/find_song_name", songData);
        const artistNameResponse = await axios.post("http://localhost:3000/get_track_artist", songData);
...
app.post("/friends_recommendation", async (req, res) => {
  ...
  try {
    ...
    const userData = {uid: userUid};
    const friendsResponse = await axios.post("http://localhost:3000/retrieve_friend_list", userData);
    ...
    let lsongIdlist = await Promise.all(
      Object.keys(liked_songs).map(async (songId) => {
        ...
        //console.log(songName.data);
        userLikedSongs.push(`${songName.data["name"]} - ${artistName.data["artist"]}`);
      })
    );
    
    for (const friendUid of friendsList) {
      const friendData = { uid: friendUid };
      const friendprivacyResponse = await axios.post("http://localhost:3000/get_privacy_value", { uid: friendUid});
      
      if (friendprivacyResponse.data["isPrivate"] === false) {
        console.log(friendUid);
        const friendTracksResponse = await axios.post("http://localhost:3000/retrieve_user_tracks", friendData);
        const likedSongs = friendTracksResponse.data[0]["liked_songs"];
        console.log(likedSongs);
       
        for (const songId in likedSongs) {
          const songNameResponse = await axios.post("http://localhost:3000/find_song_name", { docId: songId });
          const artistNameResponse = await axios.post("http://localhost:3000/get_track_artist", { docId: songId });
          allSongsFriends.push(`${songNameResponse.data["name"]} - ${artistNameResponse.data["artist"]}`);
        }
      }
    }
    ...
    const prompt = 
      'Here is my list of the songs I liked: ' + userLikedSongs + 'Also here is list of songs that my friends like: ' + allSongsFriends +
